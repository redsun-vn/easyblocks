  ...
  var checkIfValid = function checkIfValid(x) {
    ...
    if (typeof x.id === "string") {
      if (x.id.startsWith("local.")) {
        ...
        if (_typeof__default["default"](x.value) !== "object" || x.value === null) {
          return false;
        }
      }
    }
  ...
  return {
    normalize: function normalize(x) {
      if (x === undefined || x === null) {
        ...
        return {
          id: "local." + easyblocksUtils.uniqueId(),
          value: _defineProperty__default["default"]({}, compilationContext.contextParams.locale, (_schemaProp$defaultVa = schemaProp.defaultValue) !== null && _schemaProp$defaultVa !== void 0 ? _schemaProp$defaultVa : "Lorem ipsum"),
    ...
    compile: function compile(x) {
      if ("value" in x) {
        ...
        if (typeof value !== "string") {
          ...
          var fallbackValue = (_getFallbackForLocale = locales.getFallbackForLocale(x.value, compilationContext.contextParams.locale, compilationContext.locales)) !== null && _getFallbackForLocale !== void 0 ? _getFallbackForLocale : "";
          return {
            id: x.id,
            value: fallbackValue,
            widgetId: "@easyblocks/local-text"
...
var schemaPropDefinitions = {
    ...
    return {
      ...
      getHash: function getHash(value, breakpointIndex) {
        if (isTrulyResponsiveValue.isTrulyResponsiveValue(value)) {
          return responsiveValueAt.responsiveValueAt(value, breakpointIndex);
        }
        return value;
      }
    };
  ...
  "boolean": function boolean(schemaProp, compilationContext) {
    ...
    return {
      ...
      getHash: function getHash(value, breakpointIndex) {
        if (isTrulyResponsiveValue.isTrulyResponsiveValue(value)) {
          var breakpointValue = responsiveValueAt.responsiveValueAt(value, breakpointIndex);
  ...
  component: function component(schemaProp, compilationContext) {
        ...
        try {
          for (_iterator.s(); !(_step = _iterator.n()).done;) {
            ...
            if (!componentDefinition) {
              var componentDefinitionsByType = findComponentDefinition.findComponentDefinitionsByType(componentIdOrType, compilationContext);
    ...
    return {
      ...
      compile: function compile(arg, contextProps, serializedDefinitions, editingInfoComponent, configPrefix, cache) {
        ...
        // FIXME: ?????
        var _compileComponent = compileComponent.compileComponent(arg[0], compilationContext, contextProps, serializedDefinitions || {
            components: []
    ...
    return {
      ...
      compile: function compile(arr, contextProps, serializedDefinitions, editingInfoComponents, configPrefix, cache) {
        return arr.map(function (componentConfig, index) {
          var _items;
          return compileComponent.compileComponent(componentConfig, compilationContext, (contextProps.itemProps || [])[index] || {}, serializedDefinitions, cache, editingInfoComponents === null || editingInfoComponents === void 0 || (_items = editingInfoComponents.items) === null || _items === void 0 ? void 0 : _items[index], "".concat(configPrefix, ".").concat(index));
        });
      },
      getHash: function getHash(value) {
  ...
  position: function position(schemaProp, compilationContext) {
    return {
      ...
      getHash: function getHash(value, currentBreakpoint) {
        if (isTrulyResponsiveValue.isTrulyResponsiveValue(value)) {
          var breakpointValue = responsiveValueAt.responsiveValueAt(value, currentBreakpoint);
          return breakpointValue === null || breakpointValue === void 0 ? void 0 : breakpointValue.toString();
        }
        return value;
      }
  ...
  custom: function custom(schemaProp, compilationContext) {
    ...
    return {
      ...
      compile: function compile(x) {
        var val = responsiveValueMap.responsiveValueMap(x, function (y) {
          if ("value" in y) {
            return y.value;
          }
          return y;
        });
        var flattened = responsiveValueFlatten.responsiveValueFlatten(val, compilationContext.devices);
        return responsiveValueFill.responsiveValueFill(flattened, compilationContext.devices, devices.getDevicesWidths(compilationContext.devices));
      },
      getHash: function getHash(value, breakpointIndex) {
        function getTokenValue(value) {
          ...
          if (_typeof__default["default"](value.value) === "object") {
        ...
        if (isTrulyResponsiveValue.isTrulyResponsiveValue(value)) {
          ...
          return _typeof__default["default"](breakpointValue.value) === "object" ? JSON.stringify(breakpointValue.value) : breakpointValue.value;
        }
        if ("tokenId" in value) {
          return getTokenValue(value);
        }
        return _typeof__default["default"](value.value) === "object" ? JSON.stringify(value.value) : value.value;
      }
    };
  }
};
...
function getResponsiveNormalize(compilationContext, defaultValue, fallbackDefaultValue) {
  ...
  if (isTrulyResponsiveValue.isTrulyResponsiveValue(defaultValue)) {
    /**
  ...
  return function (val) {
    ...
    if (!isTrulyResponsiveValue.isTrulyResponsiveValue(val)) {
      return _defineProperty__default["default"]({
        $res: true
      }, compilationContext.mainBreakpointIndex, scalarNormalize(val));
    }
    var responsiveVal = responsiveValueMap.responsiveValueMap(val, function (x) {
...
function getFirstOptionValue(schemaProp) {
  ...
  var firstOptionValue = _typeof__default["default"](firstOption) === "object" ? firstOption.value : firstOption;
...
function normalizeComponent(configComponent, compilationContext) {
  ...
  var ret = {
    _id: (_configComponent$_id = configComponent._id) !== null && _configComponent$_id !== void 0 ? _configComponent$_id : easyblocksUtils.uniqueId(),
  ...
  if (configComponent._itemProps) {
    ...
    var _loop = function _loop(templateId) {
      ...
      var _loop2 = function _loop2(fieldName) {
        ...
        var ownerDefinition = findComponentDefinition.findComponentDefinitionById(templateId, compilationContext);
  ...
  var componentDefinition = findComponentDefinition.findComponentDefinitionById(configComponent._component, compilationContext);
  if (!componentDefinition) {
  ...
  if (ret._component === "@easyblocks/rich-text") {
    ...
    if (Object.keys(ret.elements).length === 0 || ((_ret$elements$compila = ret.elements[compilationContext.contextParams.locale]) === null || _ret$elements$compila === void 0 ? void 0 : _ret$elements$compila.length) === 0) {
      ...
      var richTextConfig = builders.buildRichTextNoCodeEntry({
...
exports.schemaPropDefinitions = schemaPropDefinitions;
