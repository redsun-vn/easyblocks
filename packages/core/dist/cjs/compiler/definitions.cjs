    ...
    compile: x => {
      if ("value" in x) {
        ...
        if (typeof value !== "string") {
          const fallbackValue = locales.getFallbackForLocale(x.value, compilationContext.contextParams.locale, compilationContext.locales) ?? "";
          return {
            id: x.id,
            value: fallbackValue,
            widgetId: "@easyblocks/local-text"
...
const schemaPropDefinitions = {
  ...
  boolean: (schemaProp, compilationContext) => {
    ...
    return {
      ...
      getHash: (value, breakpointIndex) => {
        if (isTrulyResponsiveValue.isTrulyResponsiveValue(value)) {
          const breakpointValue = responsiveValueAt.responsiveValueAt(value, breakpointIndex);
  ...
  component: (schemaProp, compilationContext) => {
    ...
    return {
      ...
      compile: (arg, contextProps, serializedDefinitions, editingInfoComponent, configPrefix, cache) => {
        ...
        } = compileComponent.compileComponent(arg[0], compilationContext, contextProps, serializedDefinitions || {
          components: []
  ...
  "component-collection": (_, compilationContext) => {
    ...
    return {
      ...
      compile: (arr, contextProps, serializedDefinitions, editingInfoComponents, configPrefix, cache) => {
        return arr.map((componentConfig, index) => compileComponent.compileComponent(componentConfig, compilationContext, (contextProps.itemProps || [])[index] || {}, serializedDefinitions, cache, editingInfoComponents?.items?.[index], `${configPrefix}.${index}`));
      },
      getHash: value => {
  ...
  position: (schemaProp, compilationContext) => {
    return {
      ...
      getHash: (value, currentBreakpoint) => {
        if (isTrulyResponsiveValue.isTrulyResponsiveValue(value)) {
          const breakpointValue = responsiveValueAt.responsiveValueAt(value, currentBreakpoint);
          return breakpointValue?.toString();
        }
        return value;
      }
  ...
  custom: (schemaProp, compilationContext) => {
    ...
    return {
      ...
      compile: x => {
        const val = responsiveValueMap.responsiveValueMap(x, y => {
          if ("value" in y) {
            return y.value;
          }
          return y;
        });
        const flattened = responsiveValueFlatten.responsiveValueFlatten(val, compilationContext.devices);
...
function getResponsiveNormalize(compilationContext, defaultValue, fallbackDefaultValue) {
  ...
  return val => {
    ...
    const responsiveVal = responsiveValueMap.responsiveValueMap(val, x => {
...
function normalizeComponent(configComponent, compilationContext) {
  ...
  if (ret._component === "@easyblocks/rich-text") {
    if (Object.keys(ret.elements).length === 0 || ret.elements[compilationContext.contextParams.locale]?.length === 0) {
      const richTextConfig = builders.buildRichTextNoCodeEntry({
...
function getSchemaDefinition(schemaProp, compilationContext) {
  const provider = compilationContext.types[schemaProp.type] && schemaProp.type !== "text" ? schemaPropDefinitions.custom :
  // @ts-expect-error
