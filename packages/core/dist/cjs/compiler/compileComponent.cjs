...
meta, cache, parentComponentEditingInfo) {
  ...
  var componentDefinition = findComponentDefinition.findComponentDefinitionById(editableElement._component, compilationContext);
  if (!componentDefinition) {
    componentDefinition = easyblocksUtils.assertDefined(findComponentDefinition.findComponentDefinitionById("@easyblocks/missing-component", compilationContext));
  ...
  if (cachedResult) {
    hasComponentConfigChanged = !easyblocksUtils.deepCompare(ownProps, cachedResult.values);
    if (!hasComponentConfigChanged) {
      ...
      configAfterAuto = easyblocksUtils.deepClone(_objectSpread(_objectSpread({}, cachedResult.valuesAfterAuto.values), cachedResult.valuesAfterAuto.params));
  ...
  if (hasComponentConfigChanged) {
    ...
    ownPropsAfterAuto = easyblocksUtils.deepClone(ownProps);
    ...
    componentDefinition.schema.forEach(function (schemaProp) {
      ...
      if (schemaProp.type === "space") {
        ...
        ownPropsAfterAuto.values[schemaProp.prop] = linearizeSpace.linearizeSpace(ownPropsAfterAuto.values[schemaProp.prop], compilationContext, $width, (_params$autoConstant = (_params = schemaProp.params) === null || _params === void 0 ? void 0 : _params.autoConstant) !== null && _params$autoConstant !== void 0 ? _params$autoConstant : DEFAULT_SPACE_AUTO_CONSTANT);
      }
    });
    itemFieldsForEach(ownPropsAfterAuto.values, compilationContext, function (arg) {
      ...
      if (arg.itemSchemaProp.type === "space") {
        ...
        value = linearizeSpace.linearizeSpace(value, compilationContext, $width, (_params$autoConstant2 = (_params2 = arg.itemSchemaProp.params) === null || _params2 === void 0 ? void 0 : _params2.autoConstant) !== null && _params$autoConstant2 !== void 0 ? _params$autoConstant2 : DEFAULT_SPACE_AUTO_CONSTANT);
      }
      easyblocksUtils.dotNotationSet(ownPropsAfterAuto.values, arg.itemPropPath, value);
    });
    ...
    for (var _prop in ownPropsAfterAuto.params) {
      ownPropsAfterAuto.params[_prop] = responsiveValueFill.responsiveValueFill(ownPropsAfterAuto.params[_prop], compilationContext.devices, devices.getDevicesWidths(compilationContext.devices));
    }
    itemFieldsForEach(ownPropsAfterAuto.values, compilationContext, function (arg) {
      easyblocksUtils.dotNotationSet(ownPropsAfterAuto.values, arg.itemPropPath, responsiveValueFill.responsiveValueFill(arg.itemPropValue, compilationContext.devices, devices.getDevicesWidths(compilationContext.devices)));
    });

    // First we compile all the props and store them in compiledValues
    var _compiledValues = compileComponentValues.compileComponentValues(ownPropsAfterAuto.values, componentDefinition, compilationContext, cache);
    compiledValues = _objectSpread(_objectSpread({}, easyblocksUtils.deepClone(ownPropsAfterAuto.values)), _compiledValues);
    ...
    itemFieldsForEach(ownPropsAfterAuto.values, compilationContext, function (_ref) {
      ...
      var compiledValue = compileFromSchema.compileFromSchema(itemPropValue, itemSchemaProp, compilationContext, cache, {}, meta);
    ...
    if (compilationContext.isEditing) {
      ...
      if (renderableComponentDefinition.editing) {
        var scalarizedConfig = resop.scalarizeConfig(compiledValues, editorContext.breakpointIndex, editorContext.devices, renderableComponentDefinition.schema);
        var identityEditingField = easyblocksUtils.assertDefined(editingInfo.fields.find(function (f) {
    ...
    var _resop = resop.resop2({
        ...
        var device = easyblocksUtils.assertDefined(compilationContext.devices.find(function (device) {
          return device.id === breakpointIndex;
        }), "Missing device \"".concat(breakpointIndex, "\""));
        var stylesInput = _objectSpread({
          values: values,
          params: _objectSpread(_objectSpread({}, params), {}, {
            $width: easyblocksUtils.assertDefined(responsiveValueAt.responsiveValueAt($width, breakpointIndex)),
            $widthAuto: easyblocksUtils.assertDefined(responsiveValueAt.responsiveValueAt($widthAuto, breakpointIndex))
          }),
    ...
    // We are going to mutate this object so let's disconnect it from its source object
    configAfterAuto = easyblocksUtils.deepClone(_objectSpread(_objectSpread({}, ownPropsAfterAuto.values), ownPropsAfterAuto.params));
  }
  if (compilationContext.isEditing) {
    /**
     * Let's build default editingOutput (fields and component output)
    ...
    if (_renderableComponentDefinition.editing) {
      var scalarizedValues = resop.scalarizeConfig(compiledValues, _editorContext.breakpointIndex, _editorContext.devices, _renderableComponentDefinition.schema);
      var _identityEditingField = easyblocksUtils.assertDefined(editingInfo.fields.find(function (f) {
        return f.prop === "$myself";
      }));
...
function addComponentToSerializedComponentDefinitions(component, meta, componentType, compilationContext) {
  ...
  var internalDefinition = findComponentDefinition.findComponentDefinition(component, compilationContext);
  var newDef = {
...
cache) {
  var componentDefinition = findComponentDefinition.findComponentDefinition(editableElement, compilationContext);
  componentDefinition.schema.forEach(function (schemaProp) {
    if (index.isSchemaPropComponentOrComponentCollection(schemaProp)) {
      ...
      var compilationOutput = compileFromSchema.compileFromSchema(editableElement[schemaProp.prop], schemaProp, compilationContext, cache, childContextProps, meta, editingInfoComponents === null || editingInfoComponents === void 0 ? void 0 : editingInfoComponents[schemaProp.prop], "".concat(configPrefix).concat(configPrefix === "" ? "" : ".").concat(schemaProp.prop));
        ...
        if (schemaProp.type === "component") {
          ...
          var configsAfterAuto = compilationOutput.map(function (compilationOutput, index) {
            if (schemaProp.itemFields) {
              ...
              easyblocksUtils.dotNotationSet(compilationOutput.configAfterAuto, itemPropsCollectionPath, itemProps);
            }
          ...
          if (schemaProp.type === "component-collection-localised") {
            ...
            configAfterAuto[schemaProp.prop] = _defineProperty__default["default"]({}, compilationContext.contextParams.locale, configsAfterAuto);
          } else {
...
function itemFieldsForEach(config, compilationContext, callback) {
  var componentDefinition = findComponentDefinition.findComponentDefinition(config, compilationContext);
  componentDefinition.schema.forEach(function (schemaProp) {
    if (index.isSchemaPropCollection(schemaProp)) {
      ...
      var value = (_dotNotationGet = easyblocksUtils.dotNotationGet(config, path)) !== null && _dotNotationGet !== void 0 ? _dotNotationGet : [];
      value.forEach(function (_, index) {
        if (itemFields) {
          itemFields.forEach(function (itemSchemaProp) {
            var itemPath = "".concat(path, ".").concat(index, ".").concat(itemSchemaProp.prop);
            var itemValue = easyblocksUtils.dotNotationGet(config, itemPath);
            callback({
...
function resolveLocalisedValue(localisedValue, compilationContext) {
  ...
  var fallbackLocale = locales.getFallbackLocaleForLocale(locale, compilationContext.locales);
  if (!fallbackLocale) {
    return;
  }
  return {
...
function buildDefaultEditingInfo(definition, configPrefix, editorContext, compiledValues, templateId) {
  var scalarizedConfig = resop.scalarizeConfig(compiledValues, editorContext.breakpointIndex, editorContext.devices, definition.schema);
  var schema = _toConsumableArray__default["default"](definition.schema);
  var defaultFields = schema
  // Right now, component-collection schema prop isn't shown in the sidebar
  .filter(function (schemaProp) {
    return !index.isSchemaPropCollection(schemaProp);
  }).filter(function (schemaProp) {
  ...
  var pathInfo = parsePath.parsePath(configPrefix, editorContext.form);
  var parentInfo = pathInfo.parent;
  if (parentInfo) {
    var parentDefinition = findComponentDefinition.findComponentDefinitionById(parentInfo.templateId, editorContext);
    if (!parentDefinition) {
    ...
    var rootComponentDefinition = easyblocksUtils.assertDefined(findComponentDefinition.findComponentDefinitionById(easyblocksUtils.dotNotationGet(editorContext.form.values, "")._component, editorContext));
  ...
  var fields = easyblocksUtils.bubbleDown(function (x) {
...
function applyEditingInfoToCompiledConfig(compiledComponentConfig, editingInfo, parentEditingInfo, widthInfo) {
  ...
  var fields = [].concat(_toConsumableArray__default["default"](headerFields), _toConsumableArray__default["default"](parentEditingInfo && "fields" in parentEditingInfo ? parentEditingInfo.fields : []), _toConsumableArray__default["default"](nonHeaderFields));
...
function compileRichTextValuesFromRichTextParts(richTextConfig, compilationContext, cache) {
  var mainColor = getMostCommonValueFromRichTextParts.getMostCommonValueFromRichTextParts(richTextConfig, "color", compilationContext, cache);
  var mainFont = getMostCommonValueFromRichTextParts.getMostCommonValueFromRichTextParts(richTextConfig, "font", compilationContext, cache);
  return {
    mainColor: mainColor,
    mainFont: mainFont
  };
...
function mapResponsiveFontToResponsiveFontSize(responsiveFontValue) {
  return Object.fromEntries(easyblocksUtils.entries(responsiveFontValue).map(function (_ref5) {
...
function addStylesHash(styles) {
  ...
  var hash = jsXxhash.xxHash32(JSON.stringify(styles));
...
function getDefaultFieldDefinition(schemaProp, configPrefix, definition, editorContext, compiledValues, templateId) {
  var tinaField = tinaFieldProviders.getTinaField(_objectSpread(_objectSpread({}, schemaProp), {}, {
    definition: definition
  }), editorContext, compiledValues[schemaProp.prop]);
  var visible = !index.isSchemaPropComponentOrComponentCollection(schemaProp);
...
function toRelativeFieldPath(path, configPrefix) {
  ...
  if (path.includes("_itemProps")) {
    ...
    var adjustedPathFragments = [].concat(_toConsumableArray__default["default"](pathFragments.slice(0, itemPropsFragmentIndex)), [pathFragments.at(-1)]);
...
function convertEditingFieldToInternalEditingField(field, internalEditingInfo, componentDefinition, editorContext, configPrefix) {
  if (componentDefinition.id === "@easyblocks/rich-text" || componentDefinition.id === "@easyblocks/rich-text-part") {
    ...
    if (Array.isArray(field.path)) {
      ...
      var fieldName = (_field$path$0$split$a = (_field$path$ = field.path[0]) === null || _field$path$ === void 0 ? void 0 : _field$path$.split(".").at(-1)) !== null && _field$path$0$split$a !== void 0 ? _field$path$0$split$a : easyblocksUtils.raiseError("Expected field name to be present");
    ...
    if (isAbsolutePath) {
      ...
      if (field.type === "fields") {
        var _field$filters;
        var groups = (_field$filters = field.filters) !== null && _field$filters !== void 0 && _field$filters.group ? easyblocksUtils.toArray(field.filters.group) : undefined;
        return {
          portal: "component",
          source: field.path,
          groups: groups
      ...
      var _fieldName = (_pathFragments$at = pathFragments.at(-1)) !== null && _pathFragments$at !== void 0 ? _pathFragments$at : easyblocksUtils.raiseError("Expected field name to be present");
  ...
  if (field.type === "field") {
    ...
    if (!sourceInternalEditingInfoField) {
      ...
      if (isPathToComponentField) {
        var componentSchemaProp = componentDefinition.schema.find(index.isSchemaPropComponentOrComponentCollection);
  ...
  if (field.type === "fields") {
    ...
    return _objectSpread({
      ...
      groups: easyblocksUtils.toArray(field.filters.group)
    });
  }
...
function isFieldPathAbsolutePath(field, editorContext) {
  var pathFragments = field.path.split(".");
  var rootValue = easyblocksUtils.dotNotationGet(editorContext.form.values, "");
  var currentPathFragmentIndex = 0;
  var currentValue = easyblocksUtils.dotNotationGet(rootValue, pathFragments[currentPathFragmentIndex]);
  while (currentValue !== undefined) {
    if (pathFragments.length - 1 === currentPathFragmentIndex) {
      return true;
    }
    currentValue = easyblocksUtils.dotNotationGet(currentValue, pathFragments[++currentPathFragmentIndex]);
  }
  return false;
}
