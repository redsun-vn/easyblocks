...
const editing = _ref => {
  ...
  const richTextPartPaths = configFindAllPaths.configFindAllPaths(richTextConfig, editorContext, config => {
  ...
  if (currentLocaleRichTextPartPaths.length === 0) {
    const fallbackLocale = locales.getFallbackLocaleForLocale(editorContext.contextParams.locale, editorContext.locales);
    if (fallbackLocale) {
