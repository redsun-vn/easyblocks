...
function buildBoxes(compiled, name, actionWrappers, meta) {
  ...
  } else if (_typeof__default["default"](compiled) === "object" && compiled !== null) {
    if (compiled.__isBox) {
      var boxProps = {
        __compiled: compiled,
        __name: name,
...
function getRenderabilityStatus(compiled, meta, externalData) {
  ...
  var requiredExternalFields = componentDefinition.schema.filter(function (schemaProp) {
    ...
    if (_typeof__default["default"](propValue) === "object" && propValue !== null && "id" in propValue && "widgetId" in propValue) {
...
function getCompiledSubcomponents(id, compiledArray, contextProps, schemaProp, path, meta, isEditing, components) {
  ...
  if (schemaProp.noInline) {
    var _elements = compiledArray.map(function (compiledChild, index) {
      return "_component" in compiledChild ? /*#__PURE__*/React__default["default"].createElement(ComponentBuilder, {
  ...
  var elements = compiledArray.map(function (compiledChild, index) {
    return "_component" in compiledChild ? /*#__PURE__*/React__default["default"].createElement(EditableComponentBuilder, {
      compiled: compiledChild,
      index: index,
  ...
  if (index.isSchemaPropComponent(schemaProp)) {
    var _elements$;
    return (_elements$ = elements[0]) !== null && _elements$ !== void 0 ? _elements$ : /*#__PURE__*/React__default["default"].createElement(React.Fragment, null);
  } else {
    return elements;
  }
}
function ComponentBuilder(props) {
  ...
  var meta = EasyblocksMetadataProvider.useEasyblocksMetadata();
  var externalData = EasyblocksExternalDataProvider.useEasyblocksExternalData();
  ...
  if (!renderabilityStatus.renderable) {
    ...
    return /*#__PURE__*/React__default["default"].createElement(MissingComponent, {
      component: componentDefinition
    }, "Fill following fields to render the component: ".concat(fieldsRequiredToRender.join(", ")), renderabilityStatus.isLoading && /*#__PURE__*/React__default["default"].createElement(React.Fragment, null, /*#__PURE__*/React__default["default"].createElement("br", null), /*#__PURE__*/React__default["default"].createElement("br", null), "Loading data..."));
  }
  ...
  var _ref = allPassedProps || {};
    ...
    restPassedProps = _objectWithoutProperties__default["default"](_ref, _excluded2);
  var runtime = {
    stitches: meta.stitches,
    resop: resop.resop,
    devices: meta.vars.devices
...
function mapExternalProps(props, configId, componentDefinition, externalData) {
  ...
  var _loop = function _loop(propName) {
    ...
    if (schemaProp) {
      ...
      !isTrulyResponsiveValue.isTrulyResponsiveValue(propValue) && _typeof__default["default"](propValue) === "object" && "id" in propValue && "widgetId" in propValue && !("value" in propValue) || isTrulyResponsiveValue.isTrulyResponsiveValue(propValue) && responsiveValueValues.responsiveValueValues(propValue).every(function (v) {
        return _typeof__default["default"](v) === "object" && v && "id" in v && "widgetId" in v && !("value" in v);
      })) {
...
function getFieldStatus(externalReference, externalData, configId, fieldName, devices) {
  return responsiveValueReduce.responsiveValueReduce(externalReference, function (currentStatus, value, deviceId) {
    if (!deviceId) {
      if (!value.id) {
        return {
          isLoading: false,
      ...
      var _externalValue = resourcesUtils.getResolvedExternalDataValue(externalData, configId, fieldName, value);
      return {
    ...
    var externalReferenceValue = responsiveValueGetDefinedValue.responsiveValueGetDefinedValue(value, deviceId, devices);
    ...
    var externalValue = resourcesUtils.getResolvedExternalDataValue(externalData, configId, fieldName, externalReferenceValue);
    return {
