...
import { uniqueId, keys, deepCompare } from '@redsun-vn/easyblocks-utils';
...
function unwrapBlockElementsNestedWithinBlockElement(editor, entry) {
  ...
  ["bulleted-list", "numbered-list", "paragraph"].includes(node.type)) {
    var nodeParent = Node.parent(editor, path);
    ...
    if (USED_IDS.has(node.id)) {
      ...
      Transforms.setNodes(editor, {
        ...
        match: function match(n) {
          return (Text.isText(n) || Element.isElement(n)) && n.id === node.id;
        }
      });
      return true;
    } else {
...
function mergeVisuallyTheSameOrEmptyTextNodes(editor, entry) {
  ...
  if (Element.isElement(node) && (node.type === "text-line" || node.type === "list-item")) {
    var textLineChildren = Array.from(Node.children(editor, path));
    if (textLineChildren.length > 1) {
      for (var childIndex = 0; childIndex < textLineChildren.length - 1; childIndex++) {
        var _textLineChildren$chi = _slicedToArray(textLineChildren[childIndex], 2),
          currentChildNode = _textLineChildren$chi[0],
          currentChildPath = _textLineChildren$chi[1];
        var _textLineChildren = _slicedToArray(textLineChildren[childIndex + 1], 2),
...
function compareText(text1, text2) {
  var areEqual = true;
  var part1Keys = keys(filterNonComparableProperties(text1));
  var part2Keys = keys(filterNonComparableProperties(text2));
  ...
  for (var index = 0; index < part1Keys.length; index++) {
    ...
    var areValuesEqual = deepCompare(part1Value, part2Value);
    if (!areValuesEqual) {
      areEqual = false;
      break;
    }
...
export { NORMALIZED_IDS_TO_IDS, withEasyblocks };
