...
function compileBox(input, devices) {
  if (_typeof(input) === "object" && input.$res) {
    var ret = {};
    ...
    return ret;
  } else if (_typeof(input) === "object" && input !== null) {
    var _ret = {};
    ...
    for (var _key in input) {
      var val = input[_key];
      if (_typeof(val) === "object" && val.$res === true) {
...
function getBoxStyles(styles, devices) {
  var flattenStyles = flattenResponsiveStyles(styles);
  var ret = {};
